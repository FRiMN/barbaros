...
class ModelManager(dict):
    ...
    Сигналы:
        added(ProviderMeta): Вызывается при добавлении нового провайдера.
        removed(str): Вызывается при удалении провайдера (передается имя).
        loaded_list_models(): Вызывается после успешной загрузки списка моделей.
        worker_started(): Вызывается при запуске рабочего потока загрузки моделей.
        worker_finished(): Вызывается при завершении рабочего потока.
        error(str): Вызывается при возникновении ошибки (передается сообщение об ошибке).
    """
